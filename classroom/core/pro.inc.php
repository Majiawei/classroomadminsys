...
function addPro(){
	...
	if($res&&$pid){
		...
		$mes="<p>添加成功!</p><a href='addPro.php' target='mainFrame'>继续添加</a>|<a href='listPro.php' target='mainFrame'>查看教室列表</a>";
	}else{
		foreach($uploadFiles as $uploadFile){
...
function getAllProByAdmin(){
	$sql="select p.id,p.pName,p.pSn,p.pNum,p.pDesc,p.pubTime,p.isShow,c.cName from room_tab as p join room_cate c on p.cId=c.id";
	$rows=fetchAll($sql);
	return $rows;
}
...
function getAllImgByProId($id){
	$sql="select a.albumPath from room_album a where pid={$id}";
	$rows=fetchAll($sql);
	return $rows;
}
...
function getProById($id){
		$sql="select p.id,p.pName,p.pSn,p.pNum,p.pDesc,p.pubTime,p.isShow,c.cName,p.cId from room_tab as p join room_cate c on p.cId=c.id where p.id={$id}";
		$row=fetchOne($sql);
		return $row;
}
/**
...
function checkProExist($cid){
	$sql="select * from room_tab where cId={$cid}";
	$rows=fetchAll($sql);
	return $rows;
}
